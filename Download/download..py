...
def generate_download_list(annotations, labelmap, base_url):
    ...
    urllist = pd.DataFrame(url_download_list)
    urllist.to_csv(r'./urllist.csv')
    print(url_download_list)
...
def main():
    ...
    df = pd.read_csv("urllist.csv")#urlist generated by download.py
    
    print(df.shape[0])

    for i in range(0, df.shape[0]-1):
        filename = str(i)
        url2 = repr(df.loc[i,"URLS"])
        url2 = "/".join(url2.split("\\\\"))
        url2 = url2[1:-1]
        dl_jpg(url2,"./imagestest/",filename)# change  "./imagestest/" to your file path
